        ...
        <div class="flex flex-col h-full lg:flex-row">
          ...
          <div class=" lg:w-80 lg:border-r lg:border-b-0">
            <div class="h-auto p-4 rounded-md bg-gray-50 lg:p-6">
              <h2 class="hidden mb-4 text-lg text-gray-800 fonht-semibold lg:mb-6 lg:text-xl">
                Account Settings
              </h2>
              <nav
                class="flex flex-row space-x-2 overflow-x-auto lg:flex-col lg:space-x-0 lg:space-y-2 lg:overflow-x-visible">
                <button v-for="tab in tabs" :key="tab.id" @click="ToogleActiveTab(tab.id)" :class="[
                  ...
                  <component :is="tab.icon" class="w-4 h-4 mb-1 lg:w-5 lg:h-5 lg:mb-0 lg:mr-3" />
                  <span class="hidden text-xs font-medium md:block lg:text-sm">{{ tab.name }}</span>
                </button>
              </nav>
            </div>
          </div>
          ...
          <div class="flex-[1] p-4 lg:p-8 overflow-auto">
            ...
            <div v-if="activeTab === 'companies'">
              <div v-if="!OpenFormCreateCompanie" class="py-1 ">
                ...
                <div v-if="companies.length < 0" class="p-6 text-center rounded-lg bg-gray-50">
                  ...
                  <button @click="OpenFormCreateCompanie = true"
                ...
                <div v-else class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                  <CompanieCard v-for="company in companies" @click="viewCompany(company.id)" :key="company.id"
                    :company="company" />
                </div>
              </div>

              <FormCreateOrUpdateCompanie @close-form="OpenFormCreateCompanie = false" v-else class="mt-4 " />
            ...
            <div v-if="activeTab === 'children'">
              ...
              <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3 lg:gap-6">
                <div v-for="child in children" :key="child.id" class="p-4 bg-white border rounded-lg shadow-sm lg:p-6">
                  <div class="flex items-center mb-3 lg:mb-4">
                    <div class="w-10 h-10 lg:w-12 lg:h-12 bg-[#fff0fb] rounded-full flex items-center justify-center">
                  ...
                  <button class="mt-3 lg:mt-4 text-[#db147f] hover:text-[#c01370] text-xs lg:text-sm font-medium">
                    Edit Details
                  </button>
                </div>
              </div>
