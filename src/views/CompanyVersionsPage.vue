    ...
    <div class="p-6 mx-auto max-w-7xl">
      ...
      <div class="mb-6">
        <nav class="flex space-x-8" aria-label="Tabs">
          ...
          <button @click="activeTab = 'companies'" :class="[
            activeTab === 'companies'
              ? 'border-[#db147f] text-[#db147f]'
              : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300',
            'whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm'
      ...
      <AppInputFiltercompanies class="my-4" :filters="searchFilters" :results-count="currentTabData.length" />

      <div v-if="activeTab === 'requests'"
        class="container grid grid-cols-1 gap-6 px-4 mx-auto mt-8 md:grid-cols-2 lg:grid-cols-3">
        <CompanyRequestCard v-for="request in filteredRequests" :key="request.id" :request="request"
          @edit="openEditDrawer" />
        <div v-if="filteredRequests.length === 0" class="py-12 text-center col-span-full">
          <div class="text-gray-500">
            ...
            <p class="mt-1 text-sm text-gray-500">Aucune demande d'entreprise ne correspond à vos critères de recherche.
            </p>
          </div>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'companies'"
        class="container grid grid-cols-1 gap-6 px-4 mx-auto mt-8 md:grid-cols-2 lg:grid-cols-3">
        <CompanyVersionCard v-for="company in filteredCompanies" :key="company.id" :version="company"
          @toggle="toggleCompanyStatus" @edit="openEditDrawer" />
        <div v-if="filteredCompanies.length === 0" class="py-12 text-center col-span-full">
          <div class="text-gray-500">
            ...
            <p class="mt-1 text-sm text-gray-500">Aucune entreprise confirmée ne correspond à vos critères de recherche.
            </p>
          </div>
        </div>
      </div>
...
import CompanyVersionCard from '@/views/CompanyVersionCard.vue'
