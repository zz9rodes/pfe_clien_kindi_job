<template>
  <div class="flex flex-col max-w-6xl px-4 mx-auto">
    <AppModal
    ...
    <!-- Header amélioré -->
    <div
      class="flex flex-col mb-8 sm:flex-row sm:items-center sm:justify-between lg:mb-10"
    >
      <div>
    ...
    <div class="p-6 mb-8 bg-white shadow-sm rounded-2xl lg:p-8">
      <div class="flex flex-col lg:flex-row lg:items-center lg:space-x-8">
        <!-- Image de profil (style conservé) -->
        <div class="flex flex-col items-center mb-6 lg:mb-0">
          <div class="relative">
            <div class="relative inline-block">
              <img
                v-if="formData.avatarUrl !== null"
                :src="
                  formData.avatarUrl || '/placeholder.svg?height=120&width=120'
                "
                :alt="auth.user.account.firstName"
        ...
        <div class="flex-1 text-center lg:text-left">
          <h2 class="mb-2 text-2xl font-bold text-gray-900 lg:text-3xl">
            {{ auth.user.account.firstName }} {{ auth.user.account.lastName }}
          </h2>
          <p class="mb-4 text-lg text-gray-600">{{ auth.user.email }}</p>

          <!-- Badges/Tags -->
          <div
            class="flex flex-wrap justify-center gap-2 mb-4 lg:justify-start"
          >
            <span
            ...
            <span
              v-if="auth.user.account.secondLangage"
          ...
          <div class="grid grid-cols-2 gap-4 lg:grid-cols-3">
            <div class="text-center lg:text-left">
              ...
              <p class="font-semibold text-gray-900">
                {{ moment(auth.user.createdAt).format("MMM YYYY") }}
              </p>
            </div>
            <div class="text-center lg:text-left">
              ...
              <p class="font-semibold text-gray-900">
                {{
                  moment(auth.user.account.updatedAt).startOf("day").fromNow()
                }}
              </p>
            </div>
          </div>
      ...
      <div class="p-6 bg-white shadow-sm rounded-2xl lg:p-8">
        ...
        <div class="space-y-6">
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              <label
                class="block mb-2 text-sm font-medium tracking-wide text-gray-500 uppercase"
                >Prénom</label
              >
          ...
          <div>
            ...
            >
            <p class="text-lg font-semibold text-gray-900">
              {{ moment(auth.user.dob).format("DD MMM YYYY") }}
            </p>
          </div>
          ...
          <div>
            ...
            >
            <p class="text-lg font-semibold text-gray-900">
              {{ auth.user.account.phoneNumber || "Non renseigné" }}
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="p-6 bg-white shadow-sm rounded-2xl lg:p-8">
        ...
        <div class="space-y-6">
          ...
          <div>
            ...
            >
            <p class="text-lg font-semibold text-gray-900">
              {{ auth.user.account.address?.title || "Non renseignée" }}
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="p-6 bg-white shadow-sm rounded-2xl lg:p-8">
        ...
        <div class="space-y-6">
          <div>
            <label
              class="block mb-2 text-sm font-medium tracking-wide text-gray-500 uppercase"
              >Langue principale</label
            >
            <div class="flex items-center">
              <span
                class="px-4 py-2 font-semibold text-blue-800 bg-blue-100 rounded-lg"
              >
                {{ auth.user.account.firstLangage }}
              </span>
            </div>
          ...
          <div v-if="auth.user.account.secondLangage">
            <label
              class="block mb-2 text-sm font-medium tracking-wide text-gray-500 uppercase"
              >Langue secondaire</label
            >
            <div class="flex items-center">
              <span
                class="px-4 py-2 font-semibold text-green-800 bg-green-100 rounded-lg"
              >
                {{ auth.user.account.secondLangage }}
              </span>
            </div>
...
<script setup>
import { CameraIcon, UserIcon, XIcon } from "lucide-vue-next";
