        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div>
              ...
              >
              <select
                v-model="localTaskData.priority"
                class="w-full px-3 py-2 text-gray-900 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#db147f] focus:border-[#db147f]"
                required
              >
            ...
            <div>
              <label class="block mb-1 text-sm font-medium text-gray-700"
                >Estimation (jours) *</label
              >
              <input
                v-model.number="localTaskData.estimationTime"
                type="number"
                min="1"
                placeholder="5"
                class="w-full px-3 py-2 text-gray-900 placeholder-gray-500 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#db147f] focus:border-[#db147f]"
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
              ...
              >
              <select
                v-model="localTaskData.assigneeId"
                class="w-full px-3 py-2 text-gray-900 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#db147f] focus:border-[#db147f]"
                required
              >
            ...
            <div v-if="localTaskData.step?.stepId">
              <label class="block mb-1 text-sm font-medium text-gray-700"
                >Étape du projet</label
              >
              <input
                disabled="true"
                v-model="localTaskData.step.name"
        ...
        <div class="space-y-4">
          <div class="flex items-center justify-between">
            ...
            <span class="text-sm text-gray-500"
              >{{ localTaskData.files.length }}/3 fichiers</span
            >
          </div>
          ...
          <div class="space-y-4">
            ...
            <div class="flex items-center gap-3">
              ...
              <button
                ...
                :disabled="localTaskData.files.length >= 3 || isUploading"
              ...
              <p class="text-sm text-gray-500">
                Maximum 3 fichiers (PDF, Images, Vidéos, DOCX) - Max 10MB par
                fichier
              </p>
            </div>
          </div>
          ...
          <div v-if="localTaskData.files.length > 0" class="space-y-3">
            ...
            <div class="space-y-2">
              <div
                v-for="(file, index) in localTaskData.files"
                :key="index"
              ...
              >
                <div class="flex items-center gap-3">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-gray-500">
                      ...
                      {{ file.type || file.fileType || "Type inconnu" }}
                    </p>
                    <!-- Barre de progression pour les nouveaux fichiers -->
                    <div
                      v-if="file.uploading"
                      class="mt-1 w-full h-1.5 bg-gray-200 rounded-full"
                    >
                      <div
                        class="bg-[#db147f] h-1.5 rounded-full transition-all duration-300"
                ...
                <div class="flex items-center gap-2">
                  ...
                  <div v-if="file.uploading" class="flex items-center gap-2">
                    <div
                      class="w-4 h-4 border-2 border-[#db147f] border-t-transparent rounded-full animate-spin"
                    ></div>
                    <span class="text-xs text-gray-500"
                      >{{ file.uploadProgress || 0 }}%</span
                    >
                  </div>
                  <CheckCircleIcon
                    v-else-if="file.uploaded !== false"
                    class="w-5 h-5 text-green-500"
                  />
                  <a
                    v-if="file.url || file.fileUrl"
          ...
          <div
            v-else
            class="p-8 text-center text-gray-500 border-2 border-gray-200 border-dashed rounded-lg"
          >
            <FolderIcon class="w-8 h-8 mx-auto mb-4 text-gray-300" />
            <p class="mb-2 text-lg font-medium text-gray-400">
      ...
      <div class="space-y-4">
        ...
        <div>
          ...
          <div class="flex justify-end mt-2">
            <button
              @click.prevent.stop="addComment"
              type="button"
              :disabled=" newComment.length < 2 ? true:false "
              :class="newComment.length < 2 ? 'bg-[#db9fbf]' : 'bg-[#db147f] hover:bg-[#c41370]'"
            ...
            >
              Ajouter un commentaire
                ...
                <div
          v-if="localTaskData.comments && localTaskData.comments.length > 0"
          class="space-y-3"
        >
          <div
            v-for="(comment, index) in localTaskData.comments"
            :key="index"
            ...
            <div class="flex justify-between mb-2">
              <div class="flex items-center gap-2">
                ...
                <span class="font-medium text-gray-900">{{
                  comment?.author?.member?.account?.firstName +" "+ comment?.author?.member?.account?.lastName || comment.user?.name || "Utilisateur"
                }}</span>
              </div>
              <span class="text-xs text-gray-500">{{
...
const normalizeExistingFiles = (files) => {
  ...
  return files.map((file) => ({
    name:
      file.name || file.fileName || file.original_name || "Fichier sans nom",
...
const uploadFile = async (fileObj) => {
  ...
  try {
    ...
    if (data.isCreate) {
      ...
      errorMessage.value =
        data.message || `Erreur lors de l'upload de ${fileObj.name}`;

      // Retirer le fichier de la liste en cas d'erreur
      const index = localTaskData.value.files.findIndex(
...
const addComment = async () => {
  if (newComment.value.trim()) {
    ...
    const taskId = localTaskData.value.slug;
    const payload = {
      text: newComment.value,
      taskId: localTaskData.value.id,
      authorId: props.connectedMember.id,
    };
    
    const response = await auth.api(
      "POST",
      `/companies/${companyId}/projects/${projectId}/tasks/${taskId}/comments/create`,
      payload,
      true
    );
    ...
    if (response.success) {
      ...
      localTaskData.value.comments.push(response.data);
    }
...
const updateTask = async () => {
  ...
  try {
    ...
    const newFiles = localTaskData.value.files
      .filter((f) => f.file !== undefined && f.uploaded === true)
      .map((f) => ({
        name: f.name,
        type: f.type,
        url: f.url,
    ...
    if (response.success) {
      ...
      errorMessage.value =
        response.message || "Erreur lors de la mise à jour de la tâche";
    }
  } catch (error) {
