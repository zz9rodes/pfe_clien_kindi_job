      ...
      <form @submit.prevent="updateProject" class="space-y-6">
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <input
              v-model="localProjectData.start"
              type="date"
              :min="today"
              class="w-full px-3 py-2 text-gray-900 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#db147f] focus:border-[#db147f]"
              required
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <select
              v-model="localProjectData.manager"
              class="w-full px-3 py-2 text-gray-900 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#db147f] focus:border-[#db147f]"
              required
            >
              <option :value="localProjectData.manager">{{localProjectData.manager}}</option>
          ...
          <div>
            ...
            <select
              v-model="localProjectData.job.title"
              class="w-full px-3 py-2 text-gray-900 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#db147f] focus:border-[#db147f]"
            >
              <option :value="localProjectData.job.title">{{localProjectData.job.title}}</option>
              <option v-for="job in jobs" :key="job.id" :value="job.title">
                {{ job.title }}
              </option>
            </select>
          </div>
        ...
        <!-- <div class="space-y-4">
          <div class="flex items-center justify-between">
            ...
            <button
              type="button"
              @click="addFile"
              :disabled="localProjectData.files.length >= 3"
          ...
          <div v-if="localProjectData.files.length === 0" class="p-4 text-center text-gray-500 border-2 border-gray-300 border-dashed rounded-lg">
          ...
          <div v-else class="space-y-3">
            <div
              v-for="(file, index) in localProjectData.files"
              :key="index"
            ...
            >
              ...
              <div class="space-y-3">
                ...
                <div v-else class="flex items-center justify-between p-3 rounded-lg bg-gray-50">
                  <div class="flex items-center gap-2">
                    <FileIcon class="w-5 h-5 text-gray-600" />
                    <span class="text-sm font-medium text-gray-900">{{ file.name || file.fileName || 'Fichier' }}</span>
                  </div>
                  <button
                    type="button"
                    @click="replaceFile(index)"
                    class="px-3 py-1 text-xs font-medium text-[#db147f] hover:underline"
                  >
                    Remplacer
                  </button>
                </div>
...
// Créer une copie locale des données du projet
...
const addFile = () => {
  if (localProjectData.value.files.length < 3) {
    localProjectData.value.files.push({
      name: '',
      type: '',
      url: '',
      fileName: ''
...
const removeFile = (index) => {
  localProjectData.value.files.splice(index, 1)
}

const replaceFile = (index) => {
  localProjectData.value.files[index].url = ''
  localProjectData.value.files[index].fileName = ''
}
...
const handleFileUpload = (event, index) => {
  ...
  if (file) {
    ...
    localProjectData.value.files[index].url = URL.createObjectURL(file)
    localProjectData.value.files[index].fileName = file.name
