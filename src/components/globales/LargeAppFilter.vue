<template>
  <div class="bg-white mx-3 rounded-md shadow-lg max-h-[80vh] overflow-y-auto modal-container">
    <!-- Header -->
    <div class="sticky top-0 z-10 flex items-center justify-between p-6 bg-white border-b border-gray-100">
      ...
      <div class="flex items-center gap-3">
        <button @click="clearAllFilters"
          class="text-sm text-[#00a3e0] hover:text-[#00a3e0]/80 font-medium transition-colors px-3 py-1 rounded">
          Clear All
        </button>
    ...
    <div class="p-6">
      <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        ...
        <div class="space-y-4">
          <button @click="toggleSection('location')" class="flex items-center justify-between w-full group">
            ...
            <div class="flex items-center gap-2">
              <span v-if="dataFilter.filters.location.length > 0" 
              ...
              <svg xmlns="http://www.w3.org/2000/svg" 
                :class="{ 'rotate-45': dataFilter.expandedSections.location }" 
                width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" 
                stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                class="text-gray-400 transition-transform group-hover:text-[#e4097f]">
                <path d="M5 12h14" />
                <path d="M12 5v14" />
              </svg>
            </div>
          ...
          <div v-if="dataFilter.expandedSections.location" class="space-y-3 overflow-y-auto max-h-48">
        ...
        <!-- Category Filter -->
        <div class="space-y-4">
          <button @click="toggleSection('category')" class="flex items-center justify-between w-full group">
            <h4 class="font-semibold text-gray-900 group-hover:text-[#e4097f] transition-colors">Category</h4>
            <div class="flex items-center gap-2">
              <span v-if="dataFilter.filters.category.length > 0" 
                class="bg-[#e4097f] text-[8px] text-white px-2 py-1 rounded-full size-[18px] flex justify-center items-center">
                {{ dataFilter.filters.category.length }}
              </span>
              <svg xmlns="http://www.w3.org/2000/svg" 
                :class="{ 'rotate-45': dataFilter.expandedSections.category }" 
                width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" 
                stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                class="text-gray-400 transition-transform group-hover:text-[#e4097f]">
                <path d="M5 12h14" />
          ...
          <div v-if="dataFilter.expandedSections.category" class="space-y-3 overflow-y-auto max-h-48">
        ...
        <!-- Salary Range Filter -->
        <div class="space-y-4">
          <button @click="toggleSection('salary')" class="flex items-center justify-between w-full group">
            <h4 class="font-semibold text-gray-900 group-hover:text-[#e4097f] transition-colors">Salary Range</h4>
            <div class="flex items-center gap-2">
              <span v-if="isSalaryFiltered" 
                class="bg-[#e4097f] text-[8px] text-white px-2 py-1 rounded-full size-[18px] flex justify-center items-center">
                1
              </span>
              <svg xmlns="http://www.w3.org/2000/svg" 
                :class="{ 'rotate-45': dataFilter.expandedSections.salary }" 
                width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" 
                stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                class="text-gray-400 transition-transform group-hover:text-[#e4097f]">
                <path d="M5 12h14" />
          ...
          <div v-if="dataFilter.expandedSections.salary" class="space-y-4">
            <div class="bg-gradient-to-r from-[#e4097f]/10 to-[#00a3e0]/10 p-4 rounded-xl">
              ...
              <div class="relative mb-4">
                ...
                <input type="range" :min="dataFilter.salaryRange.min" :max="dataFilter.salaryRange.max" 
                  v-model="dataFilter.filters.salary.min" class="absolute top-0 w-full h-2 opacity-0 cursor-pointer" />
                <input type="range" :min="dataFilter.salaryRange.min" :max="dataFilter.salaryRange.max" 
                  v-model="dataFilter.filters.salary.max" class="absolute top-0 w-full h-2 opacity-0 cursor-pointer" />
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- Job Type Filter -->
        <div class="space-y-4">
          <button @click="toggleSection('jobType')" class="flex items-center justify-between w-full group">
            <h4 class="font-semibold text-gray-900 group-hover:text-[#e4097f] transition-colors">Job Type</h4>
            <div class="flex items-center gap-2">
              <span v-if="dataFilter.filters.jobType.length > 0" 
                class="bg-[#e4097f] text-[8px] text-white px-2 py-1 rounded-full size-[18px] flex justify-center items-center">
                {{ dataFilter.filters.jobType.length }}
              </span>
              <svg :class="{ 'rotate-45': dataFilter.expandedSections.jobType }" 
          ...
          <div v-if="dataFilter.expandedSections.jobType" class="space-y-3">
            <div v-for="jobType in dataFilter.jobTypes" :key="jobType.id" class="flex items-center justify-between group">
              <div class="flex items-center">
                ...
                <span class="px-2 py-1 ml-2 text-xs text-gray-400 bg-gray-100 rounded-full">{{ jobType.count }}</span>
              </div>
        ...
        <!-- Gender Filter -->
        <div class="space-y-4">
          <button @click="toggleSection('gender')" class="flex items-center justify-between w-full group">
            <h4 class="font-semibold text-gray-900 group-hover:text-[#e4097f] transition-colors">Gender</h4>
            <div class="flex items-center gap-2">
              <span v-if="dataFilter.filters.gender.length > 0" 
                class="bg-[#e4097f] text-white text-xs px-2 py-1 rounded-full">
                {{ dataFilter.filters.gender.length }}
              </span>
              <svg :class="{ 'rotate-45': dataFilter.expandedSections.gender }" 
                xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" 
                fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                class="text-gray-400 transition-transform group-hover:text-[#e4097f]">
                <path d="M5 12h14" />
          ...
          <div v-if="dataFilter.expandedSections.gender" class="space-y-3">
      ...
      <div v-if="activeFiltersCount > 0" class="p-4 mt-6 bg-gray-50 rounded-xl">
        ...
        <div class="flex flex-wrap gap-2">
          <span v-for="filter in activeFiltersList" :key="filter.key" 
            class="inline-flex items-center px-2 py-1 bg-[#e4097f] text-white text-xs rounded-full">
            {{ filter.label }}
